...
Также создаем базовые настройки сети - блоки кода "Network", "Subet 1", "Subnet 2" в основном файле конфигурации терраформ main.tf, при этом веб-сервера размещаем в разных зонах согласно Заданию.

**Примечание**: для доступа по ssh к серверам используются ключи ed25519, как более безопасные по сравнению с RSA. Для тестирования веб серверов и балансировщика используем тестовые начальные страницы, содержимое которых формируется при развертывании веб-серверов. Устанавливаемые RPM-пакеты для обеспечения доступности при установке размещены на сервисе dropbox.  
...
Для того, чтобы не создавать дашборбы при каждом развертывании инфраструктуры в терраформ, сохраняем базу данных и настройки grafana в архивы, и добавляем в файл терраформ-конфигурации сервера grafana код загрузки предварительно сохраненной базы и настроек сервера grafana. После чего, при последующем развертывании инфраструктуры средствами терраформ, дашборды восстанавливаются.  
